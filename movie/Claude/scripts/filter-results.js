...
function linkCandidateScore(link, movie, index) {
  ...
  // Prefer links whose anchor text mentions the movie name.
  if (movie && text.indexOf(movie) !== -1) score += 70;

  // Penalize obvious category/navigation links.
  if (/\/vod\/type\//.test(href) || /\/index\.php\/vod\/type\//.test(href) ||
      /\/xigua\/\d+\/\d+\.html$/.test(href) || /\/ma\/\d+\/$/.test(href)) {
    score -= 80;
  }

  // Mild preference for earlier links to break ties, without DOM-first lock-in.
...
function linkCandidateScoreAdvanced(link, movie, index, targetYear) {
  ...
  // Prefer direct play/vodplay patterns over other link types.
...
function processData(raw) {
  ...
  // --- Step 1: Group fofo mirrors ---
  // fofo sites all return results; merge those with same movie ID on same path
  var fofoGroups = {};  // key: movie-id-path, value: { siteNames:[], best: result }
  var nonFofo = [];

  (data.results || []).forEach(function(r) {
    if (r.siteName.indexOf('fofo') === 0) {
      // Extract the movie path after domain (e.g., /dianying/59099)
      var urlParts = (r.resultUrl || '').match(/\/\w+\/\d+/);
      if (!urlParts) {
        // Try extracting from first link's href
      ...
      // Keep the result with the most links
  ...
  // Flatten fofo groups and merge mirror metadata
  Object.keys(fofoGroups).forEach(function(key) {
    var group = fofoGroups[key];
    var best = JSON.parse(JSON.stringify(group.best)); // clone
  ...
  // Add back non-fofo results
  filtered = filtered.concat(nonFofo);

  // --- Step 2: Tag each result ---
  ...
  if (summaryMode) {
    ...
    // Build one-shot verify args from all candidate URLs (copy-paste ready).
    var verifyPairs = [];
    ...
    var verifyUrlsFilePath = '$env:USERPROFILE\\.claude\\skills\\movie\\scripts\\_verify_urls.txt';
    var verifyCmdViaFile = '-UrlsFile "' + verifyUrlsFilePath + '"';
    var verifyCmdRecommended = verifyCmd.length > 800 ? verifyCmdViaFile : verifyCmd;

    // Compact output — only key fields for AI, 省 token
    var summary = {
