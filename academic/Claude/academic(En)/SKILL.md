...
**DO**:
- ✅ Start with `bash .claude/skills/academic/academic-search.sh "topic"` (default = 3 sources)
...
bash .claude/skills/academic/academic-search.sh "your topic"
...
```bash
bash .claude/skills/academic/academic-search.sh "main topic" --keywords "alternative keywords"
```

**Only if both fail and topic genuinely needs coverage:**
```bash
# Full mode — adds OpenReview, PapersWithCode, ACL Anthology
bash .claude/skills/academic/academic-search.sh "topic" --full
